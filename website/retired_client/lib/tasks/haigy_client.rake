...
namespace :haigy_client do
  ...
  task :setup, [:clobberAssets] => [:environment] do |task, args|
    ...
    puts "Running \"rake haigy_client:model:sync\""
    Rake::Task["haigy_client:model:sync"].reenable
    Rake::Task["haigy_client:model:sync"].invoke
    puts
    ...
    puts "\"rake haigy_client:setup\" is Done!"
    puts
  end
