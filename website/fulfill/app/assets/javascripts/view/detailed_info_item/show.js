modulejs.define("view/detailed_info_item/show", [
  ...
  errorHandler, cartHelper, detailedinfoitemModel, mainModalimagezoomerViewDef
) {
  "use strict";
  ...
  var detailedinfoitemShowView = Backbone.View.extend({
    ...
    render: function() {
      ...
      cachedRequest.fetchModel(detailedinfoitemModel, that.id, {
        ...
        error: function(model, jqXHR) {
          logger(jqXHR);
          errorHandler(jqXHR.responseJSON.error_code, ["[view/detailed_info_item/show] - ", jqXHR.responseJSON.error_message].join(""));
        }
      });
    ...
    subtractQuantity: function(event) {
      var that = this;
      $(event.currentTarget).blur();
      cartHelper.subtractItemQuantity(that.id, true, function() {
        that.refreshCartOperation();
      });
    },
