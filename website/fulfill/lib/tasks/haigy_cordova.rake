...
namespace :haigy_cordova do
  ...
  task deploy: :environment do
    ...
    if File.directory?(cordovaAppPath)
      ...
      puts "Running \"rake haigy_client:assets:clobber\""
      Rake::Task["haigy_client:assets:clobber"].reenable
      Rake::Task["haigy_client:assets:clobber"].invoke
      puts

      puts "Running \"rake haigy_client:assets:precompile\""
      Rake::Task["haigy_client:assets:precompile"].reenable
      Rake::Task["haigy_client:assets:precompile"].invoke
      puts
      ...
      cordovaAppHtmlFile.puts '<meta http-equiv="Content-Security-Policy" content="default-src *; style-src *; media-src *">'
      ...
      puts "Error: Cannot locate cordova app for this project. Please set a correct cordova path in the file \"config/haigy_fulfill.yml\" as: cordova_app_path: \"/Example/Cordova/App/Path\""
    end
  end
