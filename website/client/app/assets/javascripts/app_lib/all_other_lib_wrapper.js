...
modulejs.define("lib/image_zoomer", ["haigy/lib/1.1.0/image_zoomer"], function(ImageZoomer) {
  return ImageZoomer;
});
...
modulejs.define("lib/utility", ["haigy/lib/1.1.0/utility"], function(utility) {
  return utility;
});


modulejs.define("lib/validator", ["haigy/lib/1.1.0/validator"], function(validator) {
  return validator;
});
...
// usage:
// modulejs.require this module as "pageLoadingBlocker"
// var pageLoadingBlockerInstance = pageLoadingBlocker("Example Title", "Example Loading Message");
// pageLoadingBlockerInstance.open();
// pageLoadingBlockerInstance.open("New Example Title", "New Example Loading Message");
// pageLoadingBlockerInstance.close();
modulejs.define("page_loading_blocker", ["jquery", "app/constant", "haigy/lib/1.1.0/page_loading_blocker"], function($, constant, PageLoadingBlocker) {
  var pageLoadingBlockerInstance = null;

  return function(title, loadingMessage) {
    if (pageLoadingBlockerInstance && pageLoadingBlockerInstance.hasDomContainerNode()) {
      pageLoadingBlockerInstance.setTitleAndLoadingMessage(title, loadingMessage);
    } else {
      var noticeContainer = $(["#", constant.APP_LAYOUT_NOTICE_CONTAINER_HTML_ID].join(""));
      if (noticeContainer && noticeContainer.length > 0) {
        pageLoadingBlockerInstance = new PageLoadingBlocker(noticeContainer.get(0), title, loadingMessage);
      } else {
        pageLoadingBlockerInstance = new PageLoadingBlocker(null);
      }
    }
    return pageLoadingBlockerInstance;
  };
});
...
modulejs.define("stripe_checkout", function() {
